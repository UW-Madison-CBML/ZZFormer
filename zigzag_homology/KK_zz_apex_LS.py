...
def apex(pt,r,v,f):
    ...
    if not i_cone and not j_cone:
        ...
        # return lift_cycle(r[j], (birth, death), None, f, k)
    else:
        assert not i_cone and j_cone
        if i_data > j_data:     # TODO: can we check this non-numerically?
            # extended (open-open)
...
def lift_cycle(z, dir, w, fltr, k):
    ...
    for y in z:
        s = fltr[y.index]
        # print("  simplex ",s,"y is: ",y, "data is", s.data)
        if ((start < finish) and (start <= s.data <= finish)) or ((start > finish) and (start >= s.data >= finish)):
        # if ((start < finish) and (start >= s.data) and (finish >= s.data)) or ((start > finish) and (start >= s.data >= finish)):
            vertical_cells.append((s.data, (y.index, y.element)))
            a = k.id()    # +1
            for sb in s.boundary():
                ...
                if sb_idx != 0:
                    ...
                    # assert sb.dimension() == 0 and sb[0] == w
                a = k.neg(a)    # a = -a
    ...
    for idx, lst in result.items():
        for (t1,c),(t2,_) in zip(lst, lst[1:]):
            if t1 != t2 and not k.is_zero(c):
                rep.add((t1,t2), (idx,c))

    for t, data in vertical_cells:
        rep.add_vertical(t, data)
