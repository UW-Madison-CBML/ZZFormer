...
class HierarchicalCNN(nn.Module):
    ...
    def forward(self, x4, x8, x14, x20):
        ...
        z4_flat = self.flatten(self.pool(z4))
        z8_flat = self.flatten(self.pool(z8))
        z14_flat = self.flatten(self.pool(z14))
        z20_flat = self.flatten(self.pool(z20))
        # Concatenate along channel
        emb = torch.cat((z4, z8, z14, z20), dim=1)
        combined = torch.cat((z4_flat, z8_flat, z14_flat, z20_flat), dim=1)
        # Prediction
...
for fold in range(3):
    ...
    model_path = f'/staging/s/svaren/terrier_zzformer/many/{save_name}_f{fold}.pth' #Repbase_f4, RepetDB_f4
    ...
    with torch.no_grad():
        for x4, x8, x14, x20, seq, ids, label, d in chain(test_loader, train_loader):
            x4 = x4.to(device)
            x8 = x8.to(device)
            x14 = x14.to(device)
            x20 = x20.to(device)
    ...
    all_results[f"fold_{fold}"] = {'seq_z': seqs,
                                   'test_y': trues,
                                   'embeddings': z,
                                   'fold': fold}
    print(save_name)

    with open(f'/staging/svaren/terrier_zzformer/dim/{save_name}_dim_fold{fold}.pkl', 'wb') as f:
