...
class CrossAttentionFullQ(nn.Module):
    ...
    def __init__(self, d_model, n_heads, kdim=128, vdim=128, dropout=0.1, norm_first=True):
        super().__init__()
        ...
        # )
        self.cross_attn = nn.MultiheadAttention(
            embed_dim=256, num_heads=n_heads, kdim=128, vdim=128, dropout=dropout, batch_first=True
        )
...
class HierarchicalLongformerClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.full_cross_attn = nn.ModuleList([
            CrossAttentionFullQ(d_model, n_heads, kdim=self.kdim, vdim=self.vdim, dropout=dropout)
            for _ in range(num_layers)
        ])
