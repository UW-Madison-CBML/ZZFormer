...
# Train / Val
# ============================================================
# def run_train(model, topology_encoder, dataloader, optimizer, model_cfg):
def run_train(model, dataloader, optimizer, model_cfg):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        optimizer.zero_grad()
        # topology_latent_stack = [topology_encoder(img) for img in images]
        out, _ = model(
            input_ids       = input_ids,
...
@torch.no_grad()
# def run_val(model, topology_encoder, dataloader, classification_tree, model_cfg, threshold=0.0):
...
def main(args):
    ...
    mer4 = sorted(glob.glob(f"{args.pi_dir}/*_4mer.npz"), key=extract_chunk_ids)[0]
    mer8 = sorted(glob.glob(f"{args.pi_dir}/*_8mer.npz"), key=extract_chunk_ids)[0]
    mer14 = sorted(glob.glob(f"{args.pi_dir}/*_14mer.npz"), key=extract_chunk_ids)[0]
    mer20 = sorted(glob.glob(f"{args.pi_dir}/*_20mer.npz"), key=extract_chunk_ids)[0]
    ...
    model = HierarchicalLongformerClassifier(
        ...
        topology_in_channels=model_cfg["topology_in_channels"], 
        topology_cnn_filters=model_cfg["topology_cnn_filters"], 
    ...
    for epoch in range(train_cfg["epochs"]):
        ...
        # train_loss = run_train(model, topology_encoder, train_loader, optimizer, model_cfg)
    ...
    # --- Validate ---
    # val_out, res = run_val(model, topology_encoder, val_loader, classification_tree, model_cfg)
