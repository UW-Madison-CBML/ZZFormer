...
class BosCrossAttention(nn.Module):
    ...
    def forward(self, hidden, context):
        # hidden:  (B, L, d_model)
        # context: (B, N_ctx, d_model)   # N_ctx = number of topology tokens (256)
...
class HierarchicalLongformerClassifier(nn.Module):
    ...
    def _run_longformer_interleaved(
        ...
        for i, layer_module in enumerate(lf.encoder.layer):
            ...
            #    topology_latent_stack[i]: (B, 64, 256)
            #         = (B, feature_dim, num_topology_tokens)
            #    -> transpose to (B, 256, 64) so we can project per token
            #    -> Linear(64, d_model) gives (B, 256, d_model) context
            topo = topology_latent_stack[i].transpose(1, 2)      # (B, 256, 64)
            context = self.kmer_projections[i](topo)             # (B, 256, d_model)
