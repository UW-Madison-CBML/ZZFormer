...
class HierarchicalLongformerClassifier(nn.Module):
    ...
    Per layer i (k = k_mers[i]):
        ...
        3. The latent is projected to d_model and cross-attended to
           by the BOS slot only.
    ...
    def __init__(
        ...
        # ---- 2b. Per-layer topology projection + BOS cross-attention ----
    ...
    def _run_longformer_interleaved(
        self, input_ids, valid_mask, topology_images,
    ):
        lf  = self.longformer
        cfg = lf.config
        ...
        for i, layer_module in enumerate(lf.encoder.layer):
            ...
            # 2. CNN topology encoder for this k-mer:
            #    (B, C, H, W) -> (B, topology_latent_dim)
            topology_latent = self.topology_encoders[i](topology_images[i])

            # 3. Project to d_model and let BOS cross-attend to it:
            #    (B, topology_latent_dim) -> (B, 1, d_model)
            context = self.kmer_projections[i](topology_latent).unsqueeze(1)
    ...
    def forward(
        ...
        assert len(topology_images) == self.num_layers, (
            f"Expected {self.num_layers} persistence-image batches "
