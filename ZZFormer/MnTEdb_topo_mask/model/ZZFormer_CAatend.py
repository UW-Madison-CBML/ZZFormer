...
class HierarchicalLongformerClassifier(nn.Module):
    ...
    def _run_longformer_interleaved(
        self, input_ids, valid_mask, topology_latent_stack, topology_masks
    ):
        lf  = self.longformer
        cfg = lf.config
        ...
        for i, layer_module in enumerate(lf.encoder.layer):
            ...
            # 2. BOS x topology cross-attention.
            # Shape transition: (B, 64) -> (B, 1, 64)
            context = topology_latent_stack[i]#.unsqueeze(1)
            mask = topology_masks[i]
            hidden_states = self.full_cross_attn[i](
                hidden_states, 
                context,
                topo_key_padding_mask=mask
            )
    ...
    def forward(
        ...
        assert len(topology_images) == self.num_layers, "Mismatch layers/images"
        assert len(topology_masks) == self.num_layers, "Mismatch layers/masks"
