...
def run_train(model, dataloader, optimizer, model_cfg):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # topology_key_padding_mask = batch["topology_key_padding_mask"].to(DEVICE, non_blocking=True)
        ...
        single_topo_mask = batch["topology_key_padding_mask"].to(
            DEVICE, non_blocking=True
        )
        topology_masks = [single_topo_mask for _ in range(num_layers)]
        optimizer.zero_grad()
        out, _ = model(
            input_ids       = input_ids,
            attention_mask  = attention_mask,
            target_node_ids = target_node_ids,
            topology_masks  = topology_masks,
