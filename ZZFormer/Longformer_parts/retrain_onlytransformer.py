...
def main(args):
    ...
    for epoch in range(init_epoch, config["train"]["epochs"]):
        ...
        if epoch == 4 and args.pretrained_mlm:
    ...
    print(
        ...
        f"Precision SF {val_metrics[f'sf_prec_t{t_str_def}']:.4f} | "
        f"Recall SF {val_metrics[f'sf_rec_t{t_str_def}']:.4f}\n",

        f"Precision Order {val_metrics[f'order_prec_t{t_str_def}']:.4f} | "
        f"Recall Order {val_metrics[f'order_rec_t{t_str_def}']:.4f}\n",
    )
    ...
    if not args.debugging:
        wandb.log({
            ...
            # Threshold 0.7
            f"order_acc_t{t_str_def}": val_metrics[f"order_acc_t{t_str_def}"],
            f"order_f1_t{t_str_def}": val_metrics[f"order_f1_t{t_str_def}"],
            f"order_prec_t{t_str_def}": val_metrics[f"order_prec_t{t_str_def}"],
            f"order_rec_t{t_str_def}": val_metrics[f"order_rec_t{t_str_def}"],
            f"sf_acc_t{t_str_def}": val_metrics[f"sf_acc_t{t_str_def}"],
            f"sf_f1_t{t_str_def}": val_metrics[f"sf_f1_t{t_str_def}"],
            f"sf_prec_t{t_str_def}": val_metrics[f"sf_prec_t{t_str_def}"],
            f"sf_rec_t{t_str_def}": val_metrics[f"sf_rec_t{t_str_def}"],
            # Threshold 0.9
            f"order_acc_t{t_str_def}": val_metrics[f"order_acc_t{t_str_def}"],
            f"order_f1_t{t_str_def}": val_metrics[f"order_f1_t{t_str_def}"],
            f"order_prec_t{t_str_def}": val_metrics[f"order_prec_t{t_str_def}"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_mlm", type=str, required=False, help="Path to trained MLM .pt file")
