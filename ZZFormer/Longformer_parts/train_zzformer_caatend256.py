...
from model.ZZFormer_CAatend256tokens import HierarchicalLongformerClassifier,build_classification_tree
...
class TopoDataset(Dataset):
    ...
    Expects `data_dict` with:
        'sequence' : list[str]      raw nucleotide strings
        'labels'   : list           human-readable labels (passthrough)
        'label_id' : list[int]      LEAF node ids into the SoftmaxNode tree
        'x4', 'x8', 'x14', 'x20'    list/array of 64-d topology latents
                                    (each entry shape: (64,) or (64,1) or (1,64))

    Per-sample output:
        input_ids             : (L,)            long
        attention_mask        : (L,)            long, 1 = valid, 0 = pad
        target_node_ids       : ()              long
        topology_latent_stack : list of 4 tensors, each (64,)
                                in k-mer order [4, 8, 14, 20]
        labels                : passthrough
        sequence              : passthrough
        topo_mask (optional)  : (4,) float, 1 = non-zero PI, 0 = empty

    NOTE: no `global_attention_mask` is produced. The model performs
    BOS global attention internally after every cross-attention, so the
    DataLoader does not need to supply one.
    """

    def __init__(self, data_dict, max_seq_len, mask=False):
        self.sequences = data_dict['sequence']
        self.labels    = data_dict['labels']
        self.label_ids = data_dict['label_id']
        self.x4        = data_dict['x4']
        self.x8        = data_dict['x8']
        self.x14       = data_dict['x14']
        self.x20       = data_dict['x20']

        self.max_seq_len = max_seq_len
        self.mask        = mask

        assert (
            len(self.x4) == len(self.x8) == len(self.x14) == len(self.x20)
            == len(self.sequences) == len(self.label_ids)
        ), "All inputs in data_dict must have the same length."

        # ---- Pre-tokenise once: input_ids + local attention_mask ----
        ...
        print(
            ...
            f"L={max_seq_len} | k-mers=[4, 8, 14, 20] | mask={mask}"
        )

    # ------------------------------------------------------------------
    # Tokenisation (same convention as NucleotideHierarchicalDataset)
    # ------------------------------------------------------------------
    def _tokenize_all(self):
        ...
        body_max = L - 2  # reserve slots for BOS / EOS
    ...
    def __getitem__(self, idx):
        x4  = torch.as_tensor(self.x4[idx],  dtype=torch.float32)
        x8  = torch.as_tensor(self.x8[idx],  dtype=torch.float32)
        x14 = torch.as_tensor(self.x14[idx], dtype=torch.float32)
        x20 = torch.as_tensor(self.x20[idx], dtype=torch.float32)

        item = {
            "input_ids":      self.input_ids[idx],
            "attention_mask": self.attention_masks[idx],
            "target_node_ids": self.targets[idx],
            # default_collate handles list-of-tensors element-wise:
            # per-sample [(64,)]*4   --batch-->   [(B,64)]*4
            "topology_latent_stack": [x4, x8, x14, x20],
            # Passthroughs (collated to lists of length B)
        ...
        if self.mask:
            item["topo_mask"] = torch.tensor(
                [
                    1.0 if torch.any(x4  != 0) else 0.0,
                    1.0 if torch.any(x8  != 0) else 0.0,
                    1.0 if torch.any(x14 != 0) else 0.0,
                    1.0 if torch.any(x20 != 0) else 0.0,
                ],
                dtype=torch.float32,
            )
...
def run_train(model, dataloader, optimizer):
    ...
    for batch in dataloader:
        ...
        topology_latent_stack = _move_topology(batch["topology_latent_stack"], DEVICE)
...
def main(args):
    ...
    # --- Data ---
    # Pickles are now TopoDataset-style data_dicts:
    #   {'sequence': [...], 'labels': [(order, sf), ...],
    #    'x4': [...], 'x8': [...], 'x14': [...], 'x20': [...]}
    ...
    # --- Datasets (TopoDataset filters by label_to_node_id internally) ---
    ...
    # --- Model (now with topology cross-attention) ---
    model = HierarchicalLongformerClassifier(
        ...
        topology_latent_dim     = cfg["model"].get("topology_latent_dim", 64),
    ...
    train_loader = DataLoader(
        train_ds, batch_size=cfg["train"]["batchsize"],
        shuffle=True, num_workers=cfg["train"]["num_workers"],
        pin_memory=True,
    )
    val_loader = DataLoader(
        val_ds, batch_size=cfg["train"]["batchsize"],
