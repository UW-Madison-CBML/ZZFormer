...
class HierarchicalLongformerClassifier(nn.Module):
    ...
    def _run_longformer_interleaved(
        self, input_ids, valid_mask, topology_latent_stack, topo_key_padding_mask=None
    ):
        lf  = self.longformer
        cfg = lf.config
        ...
        for i, layer_module in enumerate(lf.encoder.layer):
            ...
            hidden_states = self.full_cross_attn[i](hidden_states, context, topo_key_padding_mask=topo_key_padding_mask)
    ...
    def forward(
        ...
        h = self._run_longformer_interleaved(
            ...
            topo_key_padding_mask = topo_key_padding_mask,
        )
        pooled     = h[:, 0, :]                                  # BOS pool
