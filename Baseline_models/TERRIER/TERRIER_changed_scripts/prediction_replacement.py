...
#####################################################################################################################################
        ...
        for node in self.classification_tree.node_list:
            if node.is_root:
                continue
            name = node_lineage_string(node)
            if name in category_names:
                prediction_to_column[name] = name
            else:
                # Walk up to find nearest ancestor with a column
                current = node.parent
                while current and not current.is_root:
                    parent_name = node_lineage_string(current)
                    if parent_name in category_names:
                        prediction_to_column[name] = parent_name
                        break
...
#####################################################################################################################################
        ...
        def get_prediction_probability(row):
            ...
            col = prediction_to_column.get(prediction)
        ...
        #     # while parts:
        #     #     parts.pop()
        #     #     parent_name = "/".join(parts) if parts else prediction.split("/")[0]
