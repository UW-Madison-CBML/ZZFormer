...
DATASET_NAME = sys.argv[2] if len(sys.argv) > 2 else "repbase"  # <-- set this
OUTNAME = sys.argv[3] if len(sys.argv) > 3 else "Drosophila_melanogaster"       # <-- set this
THRESHOLD = sys.argv[4] if len(sys.argv) > 4 else "0.0"                # <-- set this to match your threshold string
CM_EXT = ".CSV"
NUM_FOLDS = 5
MODEL_NAME = "Terrier"
...
def evaluate_model_across_folds(level):
    ...
    for fold in range(NUM_FOLDS):
        ...
        outdir = os.path.join(BASE_DIR, "inference_eval", OUTNAME)
...
def main():
    ...
    if summary_rows:
        ...
        output_path = os.path.join(BASE_DIR, f"{DATASET_NAME}_{OUTNAME}_thresh_{THRESHOLD}_summary_metrics.csv")
