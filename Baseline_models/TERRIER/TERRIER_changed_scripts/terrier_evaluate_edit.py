...
def predictable_labels_from_columns(data: pd.DataFrame, superfamily: bool, map: dict[str, str]) -> Set[str]:
    """
    Infer the set of labels the model can predict from the probability columns in the CSV.
    This is the closest thing evaluate.py has to the model's label set without loading a checkpoint.

    - superfamily=True  => probability columns that contain '/'
    - superfamily=False => probability columns that do NOT contain '/'

    We apply the same mapping substitutions to column names so they can be compared
    to mapped ground-truth labels.
    """
    exclude_columns = {
        'file', 'accession', 'prediction', 'probability',
        'original_id', 'original_classification', 'description'
    }
    prob_cols = [c for c in data.columns if c not in exclude_columns]
    if superfamily:
        prob_cols = [c for c in prob_cols if '/' in c]
    else:
        prob_cols = [c for c in prob_cols if '/' not in c]

    # Apply mapping to the label universe
    prob_cols = map_replace_list(prob_cols, map)
    return set(prob_cols)
...
def evaluate_results(data:pd.DataFrame, superfamily:bool=True, map:str|dict="", ignore:str|list[str]="Unknown", threshold:float|None=None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    print(f"Unique actual labels after mapping: {set(actual)}")

    print(f"Unique predicted labels after mapping: {set(predicted)}")



    predictable = predictable_labels_from_columns(data, superfamily=superfamily, map=map)
     
    # HARD-CODED BEHAVIOUR:
    # Drop sequences whose (mapped) ground-truth label is not in the set of
    # labels the model can predict (derived from probability columns).
    keep_mask = np.isin(actual, list(predictable))
    actual = actual[keep_mask]
    predicted = predicted[keep_mask]
    ...
    kept = len(actual)
    if kept == 0:
        print("Correct predictions: 0/0 (0.00%)")
    else:
        print(f"Correct predictions: {correct}/{kept} ({correct/kept:.2%})")
