...
# SUPERFAMILY_IGNORE = {"RC/RC", "RC", "Helitron", "RC/Helitron"}
SUPERFAMILY_IGNORE= set()
OTHER_LABEL = "__OTHER__"


def restrict_to_gt_classes_bucket_pred_only(cm_df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    """
    Keep only classes that appear in ground truth (rows with sum>0) as the macro label set.
    Predictions into any non-GT class are bucketed into OTHER_LABEL so they are penalized.

    Returns:
      cm_eval: rows=active GT classes, cols=active GT classes (+ OTHER_LABEL if needed)
      gt_classes: list of GT-active classes (used for macro averaging)
    """
    # Make sure rows/cols cover the same label universe
    all_labels = sorted(set(cm_df.index) | set(cm_df.columns))
    cm_df = cm_df.reindex(index=all_labels, columns=all_labels, fill_value=0)

    # Active GT classes: those with any true samples
    ...
    # Columns that are NOT GT classes (predicted-only relative to this fold's GT)
    ...
    # Ensure all GT columns exist (even if never predicted) + OTHER if created
...
def metrics_from_confusion_matrix(cm_path, level="order"):
    cm_df = pd.read_csv(cm_path, index_col=0)

    # For superfamily, drop artifact classes from both rows and columns
    if level == "superfamily":
        ignore = SUPERFAMILY_IGNORE & (set(cm_df.index) | set(cm_df.columns))
        if ignore:
            print(f"  [INFO] Dropping RC/Helitron artifact classes: {ignore}")
            cm_df = cm_df.drop(index=[c for c in ignore if c in cm_df.index], errors="ignore")
            cm_df = cm_df.drop(columns=[c for c in ignore if c in cm_df.columns], errors="ignore")

    # NEW behavior: restrict macro labels to GT-present classes, but penalize pred-only classes via OTHER bucket
    ...
    # Reconstruct y_true and y_pred from cm_eval
    # y_true uses only GT classes (rows)
    # y_pred can be GT classes OR OTHER_LABEL
    y_true, y_pred = [], []
    row_labels = list(cm_eval.index)
    col_labels = list(cm_eval.columns)

    for i, true_class in enumerate(row_labels):
        for j, pred_class in enumerate(col_labels):
    ...
    # Macro metrics over GT classes ONLY
    macro_precision = precision_score(
        y_true, y_pred, labels=gt_classes, average="macro", zero_division=0
    )
    ...
    # Accuracy includes OTHER predictions as wrong (penalized)
    ...
    return {
        ...
        "has_other_bucket": (OTHER_LABEL in col_labels),
    }
...
def evaluate_model_across_folds(level):
    ...
    outdir = os.path.join(BASE_DIR, "validation", DATASET_NAME)
    ...
    for fold in range(NUM_FOLDS):
        ...
        metrics = metrics_from_confusion_matrix(cm_path, level=level)
        if metrics is None:
...
def print_model_results(model_name, level, fold_metrics, available_folds):
    ...
    for fold_idx, m in zip(available_folds, fold_metrics):
        print(f"    Fold {fold_idx}: "
              ...
              f"(GT-classes={m['num_gt_classes']}, OTHER={m['has_other_bucket']})")
...
def main():
    ...
    # ORDER
    print("=" * 70)
    print("ORDER-LEVEL RESULTS (averaged across folds)")
    print("=" * 70)
    ...
    # SUPERFAMILY
    print("\n" + "=" * 70)
    print("SUPERFAMILY-LEVEL RESULTS (averaged across folds)")
    print("=" * 70)
    ...
    if summary_rows:
        ...
        output_path = os.path.join(
            BASE_DIR,
            f"5crossVAL_{DATASET_NAME}_thresh_{THRESHOLD}_summary_metrics.csv",
        )
...
'''
python3 compute_metrics_validation.py /staging/kkumari/TERRIER repbase  0.7
python3 compute_metrics_validation.py /staging/kkumari/TERRIER repbase  0.9


python3 compute_metrics_validation.py /staging/kkumari/TERRIER mntedb  0.7
python3 compute_metrics_validation.py /staging/kkumari/TERRIER mntedb  0.9

python3 compute_metrics_validation.py /staging/kkumari/TERRIER repetdb  0.7
python3 compute_metrics_validation.py /staging/kkumari/TERRIER repetdb  0.9
