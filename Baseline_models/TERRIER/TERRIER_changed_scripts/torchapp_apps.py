...
class TorchApp(Citable, CLIApp):
    ...
    def load_checkpoint(
        ...
        **kwargs,
        ...
        return module_class.load_from_checkpoint(str(path), weights_only=False)
    ...
    # ) -> "LightningModule":
    #     """
    #     Load a checkpoint by:
    #     1) building app state (setup/data) so tree-dependent attributes exist
    #     2) instantiating the LightningModule via self.lightning_module()
    #     3) torch.load() the checkpoint and load ckpt['state_dict'] into the module

    #     This avoids Lightning's load_from_checkpoint() pickling path and lets the app
    #     control model construction (e.g. classification_tree exists).
    #     """
    #     import torch
    ...
    #     # 2) Build app state FIRST if the app has a setup_and_data method.
    #     #    For Terrier, this is what ensures classification_tree is created.
    #     try:
    #         self.setup_and_data(**kwargs)
    #     except Exception:
    #         # Some apps won't need/implement setup_and_data. That's fine.
    #         pass

    #     # 3) Construct a fresh module from current app config
    #     module = self.lightning_module(**kwargs)

    #     # 4) Load checkpoint state_dict into the module (trusted checkpoint)
