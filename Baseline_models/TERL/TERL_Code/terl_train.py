...
def _is_fasta(name):
    return (not name.startswith('.')) and name.lower().endswith(
        ('.fa', '.fasta', '.fna', '.txt')
    )

def get_files(root):
    train_files = [root + '/Train/' + f for f in os.listdir(root + '/Train') if _is_fasta(f)]
    test_files  = [root + '/Test/'  + f for f in os.listdir(root + '/Test')  if _is_fasta(f)]
    return train_files, test_files
...
m = metrics.Metric(labels, best_result[1], classes=db.classes, filename_prefix=options.prefix,  output_dir=options.output_dir,)
...
if options.save_report:
    ...
    report_path = os.path.join(
        options.output_dir,
        f'Report_{options.prefix}_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    )
