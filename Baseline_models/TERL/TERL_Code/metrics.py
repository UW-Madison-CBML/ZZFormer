...
class Metric:
    ...
    def sklearn_report(self, digits: int = 4) -> str:
        """Compute precision/recall/F1/accuracy with sklearn (macro, micro, weighted)."""
        y_true = self.labels
        ...
        p_micro   = precision_score(y_true, y_pred, labels=labels, average='micro',    zero_division=0)
        r_micro   = recall_score   (y_true, y_pred, labels=labels, average='micro',    zero_division=0)
        f1_micro  = f1_score       (y_true, y_pred, labels=labels, average='micro',    zero_division=0)
        p_weight  = precision_score(y_true, y_pred, labels=labels, average='weighted', zero_division=0)
        r_weight  = recall_score   (y_true, y_pred, labels=labels, average='weighted', zero_division=0)
        f1_weight = f1_score       (y_true, y_pred, labels=labels, average='weighted', zero_division=0)

        out  = '\n' + '*' * 79 + '\n'
        out += '**' + ' ' * 22 + ' SKLEARN CLASSIFICATION REPORT ' + ' ' * 22 + '**\n'
        out += '*' * 79 + '\n'
        out += f'Accuracy:        {acc:.{digits}f}\n\n'
        out += f'{"Average":10s} {"Precision":>10s} {"Recall":>10s} {"F1":>10s}\n'
        out += f'{"macro":10s} {p_macro:10.{digits}f} {r_macro:10.{digits}f} {f1_macro:10.{digits}f}\n'
        out += f'{"micro":10s} {p_micro:10.{digits}f} {r_micro:10.{digits}f} {f1_micro:10.{digits}f}\n'
        out += f'{"weighted":10s} {p_weight:10.{digits}f} {r_weight:10.{digits}f} {f1_weight:10.{digits}f}\n\n'
        out += 'Per-class report (sklearn):\n'
        out += classification_report(y_true, y_pred, labels=labels, target_names=target_names, digits=digits, zero_division=0) + '\n'
        out += '\nConfusion matrix (sklearn, row=true, col=pred):\n'
        out += str(confusion_matrix(y_true, y_pred, labels=labels)) + '\n'
        return out
