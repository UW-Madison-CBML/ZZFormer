...
class LazyTopoDataset(Dataset):
    """
    Memory-efficient Dataset for HierarchicalLongformerClassifier.
    Caches memory-mapped NPZ handles to eliminate file open/close overhead.
    """
    def __init__(
        self,
        file_quads,
        ...
        # Dictionary cache for open np.load handles: { file_path: npz_archive }
        self._archive_cache = {}
        ...
        for quad in file_quads:
            path_map = dict(zip(self.k_mers, quad))
            ...
            for row_idx, m in enumerate(metadata):
                ...
                if split == 'train' and fold_val in ('train', f'fold_{fold_idx}', str(fold_idx)):
                    keep = True
                elif split in ('val', 'test') and fold_val in ('test', 'val', f'fold_{fold_idx}', str(fold_idx)):
                    keep = True
                else:
                    keep = False

                if keep:
                    label_str = str(m[1]).strip()
                    label_id = label_map.get(label_str)
                    if label_id is not None:
                        self.samples.append({
                            "paths": path_map,
    ...
    @property
    def label_ids(self):
        return [sample["label_id"] for sample in self.samples]

    def _get_archive(self, file_path):
        """Fetches cached handle or opens memory-mapped file once."""
        if file_path not in self._archive_cache:
            self._archive_cache[file_path] = np.load(file_path, mmap_mode='r')
        return self._archive_cache[file_path]
    ...
    def __getitem__(self, idx):
        if isinstance(idx, str):
            if idx in ('label_ids', 'target_node_ids'):
                return self.label_ids
            raise KeyError(f"Invalid key '{idx}' for LazyTopoDataset")

        sample = self.samples[idx]
        seq = sample["sequence"]
        r_idx = sample["row_idx"]
        paths = sample["paths"]
        ...
        for k in self.k_mers:
            ...
            # Fetch from cached archive without closing file handle
            archive = self._get_archive(file_path)
            key = archive.files[0]
            img_arr = archive[key][r_idx]
    ...
    def close_archives(self):
        """Optional explicit cleanup of open file handles."""
        for archive in self._archive_cache.values():
            try:
                archive.close()
            except Exception:
                pass
